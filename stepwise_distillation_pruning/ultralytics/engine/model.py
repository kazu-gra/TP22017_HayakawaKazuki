...
class Model(nn.Module):
    ...
    def train(self, trainer=None, **kwargs):
        ...
        if not args.get('resume'):  # manually set model only if not resuming
            self.trainer.model = deepcopy(self.model)#self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
            self.model = self.trainer.model.cpu()

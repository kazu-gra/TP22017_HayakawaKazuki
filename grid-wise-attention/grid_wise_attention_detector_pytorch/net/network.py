...
from retina.anchors import Anchors
from retina.loss import FocalLoss
from retina.utils import BBoxTransform, ClipBoxes
...
class Network(nn.Module):
    def __init__(self, input_size, phase='train', model='anchor', scale='single', num_classes=1):
        ...
        resnet = resnet50(pretrained=True, receptive_keep=False)
        self.phase = phase
        ...
        if self.detection_scale == 'multi':
            ...
            self.localization_branch_l = local_branch(2048, gh_l, gw_l, phase)
    ...
    def forward(self, input, gt_boxes=None, im_info=None):
        ...
        ##################################################################################
            ...
            #print(batch_size, width, height, self.n_classes)
            #print(x_cls_1.shape)
            out2 = x_cls_1.view(batch_size, width, height, 3, self.n_classes)
            classification = out2.contiguous().view(x.shape[0], -1, self.n_classes) #x.shape[0]:batchsize, -1:残りのもの（w*h*アンカー数）16*8*3=384
            ...
            if self.detection_scale == 'single':
                ...
                print(classification.size(), classification_l.size(), classification_s.size())
